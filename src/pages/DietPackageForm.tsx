  ...
  const MealSection: React.FC<{ title: string; mealType: string; items: MealItem[] }> = ({
    ...
    <div>
      <div className="meal-header">
        <h4>{title}</h4>
        <button type="button" className="add" onClick={() => addMealItem(mealType)}>Add Item</button>
      </div>
      {items.map((item, index) => (
        <div key={index} className="meal-row">
          ...
          />
          <span>{item.unit}</span>
          <button type="button" className="danger" onClick={() => removeMealItem(mealType, index)}>X</button>
        </div>
      ))}
    </div>
  );
